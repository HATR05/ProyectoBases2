...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO barrio (nombre_barrio, ciudad_id) VALUES (%s, %s)",
...
mysql_select_db($database_connect_DB, $connect_DB);
$query_bar1 = "SELECT ciudad_id FROM ciudad";
$bar1 = mysql_query($query_bar1, $connect_DB) or die(mysql_error());
$row_bar1 = mysql_fetch_assoc($bar1);
$totalRows_bar1 = mysql_num_rows($bar1);
?>
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
        <div class="wrapper">
            ...
            <div id="content" class="background-page">
                ...
                <div class="contenido">
                    <h1 class="titleCreate">Agregar nuevo Barrio.</h1>
                                        <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
                      <table align="center">
                        <tr valign="baseline">
                          <td nowrap align="right">Nombre_barrio:</td>
                          <td><input type="text" name="nombre_barrio" value="" size="90" class="form-control"></td>
                        </tr>
                        <tr valign="baseline">
                          <td nowrap align="right">Ciudad_id:</td>
                          <td><select name="ciudad_id" class="form-control">
                            <?php
do {  
?>
                            <option value="<?php echo $row_bar1['ciudad_id']?>"><?php echo $row_bar1['ciudad_id']?></option>
                            <?php
} while ($row_bar1 = mysql_fetch_assoc($bar1));
  $rows = mysql_num_rows($bar1);
  if($rows > 0) {
      mysql_data_seek($bar1, 0);
	  $row_bar1 = mysql_fetch_assoc($bar1);
  }
?>
                          </select></td>
